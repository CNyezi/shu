...
- **唤起 / 隐藏**：默认热键 `Cmd+Shift+Space`，可在 `/设置` 中修改（失焦或 `Esc` 自动隐藏）。
- **启动应用**：输入应用名（如 `saf`）→ `↑/↓` 选择 → `Enter` 启动（结果带应用图标）。支持拼音全拼 / 首字母（`wx` → 微信）与模糊匹配（`vsc` → Visual Studio Code），按使用频率排序。
- **内容感知**：唤起时读一次剪贴板并识别类型。若内容是 JSON：只有一个适配插件时**直接打开**，多个时在列表顶部带图标推荐供选择。同一段剪贴板内容只自动打开一次；可在插件管理中按插件关闭自动打开。
- **命令**：输入 `/` 调用软件能力——`/插件`（插件管理）、`/设置`（热键等设置）。
